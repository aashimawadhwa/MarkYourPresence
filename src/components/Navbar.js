							...
							{currentUser._id
								? NavbarDataTeacher.map((item, index) => (
										<li className="items-center">
											<Link
												className="text-white rounded-r-xl hover:bg-blue-300 text-base py-3 pl-5 mb-6 font-bold block items-center  whitespace-nowrap space-x-3"
												to={item.path}
											>
												<span>{item.icon}</span>
								...
								: NavbarDataStudent.map((item, index) => (
										<li className="items-center">
											<Link
												className="text-blue-400 hover:text-blue-600 text-lg uppercase py-3 font-bold block items-center justify-between whitespace-nowrap space-x-3"
												to={item.path}
											>
												<span>{item.icon}</span>
												<span>{item.title}</span>
